...
const { getImageGalleryWithoutURl2Model, updateImageGalleryWithURL2Model } = require("../models/gallery.model");
...
const uploadGalleryImages = async (req, res) => {
    try {
        ...
        const [imgWhitoutURL2] = await getImageGalleryWithoutURl2Model();
        ...
        if(imgWhitoutURL2.length !== 0){
            const migrated = await Promise.all(imgWhitoutURL2.slice(0, 50).map(async (image) => {
                ...
                if (!response.ok) {
                    ...
                    // Insertar en la BD, en su correspondiente fila
                    await updateImageGalleryWithURL2Model(image.id, {img_url2});
